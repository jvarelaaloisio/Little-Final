...
Shader "TriplanarTerrain"
{
	...
	SubShader
	{
		...
		inline float4 TriplanarSampling11_g39( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
		...
		inline float4 TriplanarSampling6_g39( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
			float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
			projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
			float3 nsign = sign( worldNormal );
		...
		inline float4 TriplanarSampling11_g40( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
			float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
			projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
			float3 nsign = sign( worldNormal );
		...
		inline float4 TriplanarSampling6_g40( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
			float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
			projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
			float3 nsign = sign( worldNormal );
		...
		inline float4 TriplanarSampling11_g37( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
			float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
			projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
			float3 nsign = sign( worldNormal );
		...
		inline float4 TriplanarSampling6_g37( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
			float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
			projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
			float3 nsign = sign( worldNormal );
		...
		inline float4 TriplanarSampling11_g38( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
			float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
			projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
			float3 nsign = sign( worldNormal );
		...
		inline float4 TriplanarSampling6_g38( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
		{
			float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
			projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
			float3 nsign = sign( worldNormal );
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float temp_output_13_0_g39 = _CliffTiling;
			float2 temp_cast_0 = (temp_output_13_0_g39).xx;
			float3 ase_worldPos = i.worldPos;
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float4 triplanar11_g39 = TriplanarSampling11_g39( _Cliffs, ase_worldPos, ase_worldNormal, 1.0, temp_cast_0, 1.0, 0 );
			float2 temp_cast_1 = (( temp_output_13_0_g39 * _SecondaryCliffTilingFactor )).xx;
			float4 triplanar6_g39 = TriplanarSampling6_g39( _Cliffs, ase_worldPos, ase_worldNormal, 1.0, temp_cast_1, 1.0, 0 );
			float4 lerpResult5_g39 = lerp( triplanar11_g39 , triplanar6_g39 , _CliffTilingBlend);
			float4 Cliff_Blend123 = lerpResult5_g39;
			float temp_output_13_0_g40 = _GroundTiling;
			float2 temp_cast_2 = (temp_output_13_0_g40).xx;
			float4 triplanar11_g40 = TriplanarSampling11_g40( _Ground, ase_worldPos, ase_worldNormal, 1.0, temp_cast_2, 1.0, 0 );
			float2 temp_cast_3 = (( temp_output_13_0_g40 * _SecondaryGroundTilingFactor )).xx;
			float4 triplanar6_g40 = TriplanarSampling6_g40( _Ground, ase_worldPos, ase_worldNormal, 1.0, temp_cast_3, 1.0, 0 );
			float4 lerpResult5_g40 = lerp( triplanar11_g40 , triplanar6_g40 , _GroundTilingBlend);
			float4 GroundBlend125 = lerpResult5_g40;
			...
			float temp_output_13_0_g37 = _DownWallsTiling;
			float2 temp_cast_4 = (temp_output_13_0_g37).xx;
			float4 triplanar11_g37 = TriplanarSampling11_g37( _DownWalls, ase_worldPos, ase_worldNormal, 1.0, temp_cast_4, 1.0, 0 );
			float2 temp_cast_5 = (( temp_output_13_0_g37 * _SecondaryDownWallsTilingFactor )).xx;
			float4 triplanar6_g37 = TriplanarSampling6_g37( _DownWalls, ase_worldPos, ase_worldNormal, 1.0, temp_cast_5, 1.0, 0 );
			float4 lerpResult5_g37 = lerp( triplanar11_g37 , triplanar6_g37 , _DownWallsBlend);
			float4 CliffDownBlend137 = lerpResult5_g37;
			float4 lerpResult23 = lerp( lerpResult10 , CliffDownBlend137 , break186.y);
			float temp_output_13_0_g38 = _SlopeTiling;
			float2 temp_cast_6 = (temp_output_13_0_g38).xx;
			float4 triplanar11_g38 = TriplanarSampling11_g38( _Slope, ase_worldPos, ase_worldNormal, 1.0, temp_cast_6, 1.0, 0 );
			float2 temp_cast_7 = (( temp_output_13_0_g38 * _SecondarySlopeTilingFactor )).xx;
			float4 triplanar6_g38 = TriplanarSampling6_g38( _Slope, ase_worldPos, ase_worldNormal, 1.0, temp_cast_7, 1.0, 0 );
			float4 lerpResult5_g38 = lerp( triplanar11_g38 , triplanar6_g38 , _Slopeblend);
			float4 SlopeBlend139 = lerpResult5_g38;
...
/*ASEBEGIN
Version=19103
Node;AmplifyShaderEditor.CommentaryNode;200;2560,-1344;Inherit;False;561;521;;2;199;0;OUT;0,1,0,0.5019608;0;0
...
Node;AmplifyShaderEditor.CommentaryNode;141;-2560,-960;Inherit;False;868.561;579.5876;Slope;6;136;224;218;82;139;85;;1,1,1,0.2;0;0
Node;AmplifyShaderEditor.CommentaryNode;134;-2560,-1568;Inherit;False;864.8318;545.8157;Cliff Down;6;137;217;223;133;80;84;;1,1,1,0.2;0;0
Node;AmplifyShaderEditor.CommentaryNode;128;-2560,-2144;Inherit;False;871.171;544.9988;Ground;6;221;125;220;116;83;78;;1,1,1,0.2;0;0
Node;AmplifyShaderEditor.CommentaryNode;127;-2560,-2720;Inherit;False;869.4387;545.7482;Cliff;6;123;219;130;222;75;76;;1,1,1,0.2;0;0
...
Node;AmplifyShaderEditor.FunctionNode;168;-1088,-2368;Inherit;False;SaturateSubtract;-1;;17;7fec0e09a134197478345092e0bdc7a3;0;2;4;FLOAT;0;False;5;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;173;-1088,-2176;Inherit;False;SaturateSubtract;-1;;19;7fec0e09a134197478345092e0bdc7a3;0;2;4;FLOAT;0;False;5;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.BreakToComponentsNode;186;-992,-832;Inherit;False;FLOAT3;1;0;FLOAT3;0,0,0;False;16;FLOAT;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT;5;FLOAT;6;FLOAT;7;FLOAT;8;FLOAT;9;FLOAT;10;FLOAT;11;FLOAT;12;FLOAT;13;FLOAT;14;FLOAT;15
...
Node;AmplifyShaderEditor.RangedFloatNode;56;-2528,544;Inherit;False;Property;_SlopeOffset;Slope Offset;15;0;Create;True;0;0;0;False;0;False;0;2.59;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;85;-2528,-672;Inherit;False;Property;_SlopeTiling;Slope Tiling;13;0;Create;True;0;0;0;False;0;False;0;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;78;-2528,-2080;Inherit;True;Property;_Ground;Ground;0;3;[Header];[NoScaleOffset];[SingleLineTexture];Create;True;1;Ground SetUp;0;0;False;1;Space(10);False;None;82775c992a97da846beeb308f76030c4;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.TexturePropertyNode;75;-2528,-2656;Inherit;True;Property;_Cliffs;Cliffs;8;3;[Header];[NoScaleOffset];[SingleLineTexture];Create;True;1;Cliff SetUp;0;0;False;1;Space(10);False;None;06d2f3147da96854fa9f6b36b4b3616c;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.TexturePropertyNode;80;-2528,-1504;Inherit;True;Property;_DownWalls;Down Walls;4;3;[Header];[NoScaleOffset];[SingleLineTexture];Create;True;1;Down Walls Set Up;0;0;False;1;Space(10);False;None;25e1d1375f6e5424fa4152cdc64a746c;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.TexturePropertyNode;82;-2528,-896;Inherit;True;Property;_Slope;Slope;12;3;[Header];[NoScaleOffset];[SingleLineTexture];Create;True;1;Slope Set Up;0;0;False;1;Space(10);False;None;c29ec51bd7874d647a62066d39c3d806;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.FunctionNode;220;-2176,-1920;Inherit;False;TriplanarTilingBlend;-1;;40;3af183405f467664aa5662047cedce3a;0;4;12;SAMPLER2D;0;False;13;FLOAT;0;False;17;FLOAT;0.01;False;16;FLOAT;0;False;1;FLOAT4;0
...
Node;AmplifyShaderEditor.RangedFloatNode;130;-2528,-2272;Inherit;False;Property;_CliffTilingBlend;Cliff Tiling Blend;11;0;Create;True;0;0;0;False;0;False;0.5;0.5;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;219;-2208,-2496;Inherit;False;TriplanarTilingBlend;-1;;39;3af183405f467664aa5662047cedce3a;0;4;12;SAMPLER2D;0;False;13;FLOAT;0;False;17;FLOAT;0.5;False;16;FLOAT;0;False;1;FLOAT4;0
...
Node;AmplifyShaderEditor.RangedFloatNode;133;-2528,-1104;Inherit;False;Property;_DownWallsBlend;Down Walls Blend;7;0;Create;True;0;0;0;False;0;False;0.5;0.486;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;217;-2176,-1296;Inherit;False;TriplanarTilingBlend;-1;;37;3af183405f467664aa5662047cedce3a;0;4;12;SAMPLER2D;0;False;13;FLOAT;0;False;17;FLOAT;0.5;False;16;FLOAT;0;False;1;FLOAT4;0
...
Node;AmplifyShaderEditor.RangedFloatNode;136;-2528,-480;Inherit;False;Property;_Slopeblend;Slope blend;16;0;Create;True;0;0;0;False;0;False;0.5;0.5;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;218;-2176,-704;Inherit;False;TriplanarTilingBlend;-1;;38;3af183405f467664aa5662047cedce3a;0;4;12;SAMPLER2D;0;False;13;FLOAT;0;False;17;FLOAT;0.5;False;16;FLOAT;0;False;1;FLOAT4;0
...
WireConnection;220;12;78;0
WireConnection;220;13;83;0
WireConnection;220;17;221;0
WireConnection;220;16;116;0
WireConnection;125;0;220;0
WireConnection;219;12;75;0
WireConnection;219;13;76;0
WireConnection;219;17;222;0
WireConnection;219;16;130;0
WireConnection;123;0;219;0
WireConnection;217;12;80;0
WireConnection;217;13;84;0
WireConnection;217;17;223;0
WireConnection;217;16;133;0
WireConnection;137;0;217;0
WireConnection;218;12;82;0
WireConnection;218;13;85;0
WireConnection;218;17;224;0
WireConnection;218;16;136;0
WireConnection;139;0;218;0
ASEEND*/
//CHKSM=BC665C4B8B84BF9AF26337AE3689D89601098D45