...
Shader "Drimys/Nature/Leaves"
{
	Properties
	{
		...
		[ASEEnd]_AlphaMaskValue("Alpha Mask Value", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float mulTime311 = _TimeParameters.x * WindSpeed;
				float4 appendResult312 = (float4(mulTime311 , 0.0 , 0.0 , 0.0));
				float2 texCoord310 = v.texcoord.xy * float2( 1,1 ) + appendResult312.xy;
				float simplePerlin2D309 = snoise( texCoord310*(0.5 + (WindTurbulence - 0.0) * (0.7 - 0.5) / (1.0 - 0.0)) );
				...
				float4 transform262 = mul(GetWorldToObjectMatrix(),float4( ( appendResult300 * (0.0 + (v.ase_color.r - 0.0) * (_LocalWindMask - 0.0) / (1.0 - 0.0)) ) , 0.0 ));
				...
				float3 vertexValue = OUT_VertexOffset384.xyz;
						 ...
						 ) : SV_Target
			{
				...
				float3 BaseColor = OUT_BaseColor382;
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v )
			{
				...
				float mulTime311 = _TimeParameters.x * WindSpeed;
				float4 appendResult312 = (float4(mulTime311 , 0.0 , 0.0 , 0.0));
				float2 texCoord310 = v.ase_texcoord.xy * float2( 1,1 ) + appendResult312.xy;
				float simplePerlin2D309 = snoise( texCoord310*(0.5 + (WindTurbulence - 0.0) * (0.7 - 0.5) / (1.0 - 0.0)) );
				simplePerlin2D309 = simplePerlin2D309*0.5 + 0.5;
				float temp_output_306_0 = (0.0 + (simplePerlin2D309 - 0.0) * (WindIntensity - 0.0) / (1.0 - 0.0));
				float3 appendResult257 = (float3(WindDirX , ( ( ( abs( WindDirX ) + abs( WindDirZ ) ) / 4.0 ) * -1.0 ) , WindDirZ));
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float mulTime311 = _TimeParameters.x * WindSpeed;
				float4 appendResult312 = (float4(mulTime311 , 0.0 , 0.0 , 0.0));
				float2 texCoord310 = v.texcoord0.xy * float2( 1,1 ) + appendResult312.xy;
				float simplePerlin2D309 = snoise( texCoord310*(0.5 + (WindTurbulence - 0.0) * (0.7 - 0.5) / (1.0 - 0.0)) );
				simplePerlin2D309 = simplePerlin2D309*0.5 + 0.5;
				float temp_output_306_0 = (0.0 + (simplePerlin2D309 - 0.0) * (WindIntensity - 0.0) / (1.0 - 0.0));
				float3 appendResult257 = (float3(WindDirX , ( ( ( abs( WindDirX ) + abs( WindDirZ ) ) / 4.0 ) * -1.0 ) , WindDirZ));
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				surfaceDescription.Alpha = OUT_Opacity383;
...
Node;AmplifyShaderEditor.RangedFloatNode;267;-1058.231,1619.064;Inherit;False;Global;WindDirX;Wind Dir X;3;0;Create;True;0;0;0;False;0;False;0;0;-1;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;268;-1065.849,1753.373;Inherit;False;Global;WindDirZ;Wind Dir Z;4;0;Create;True;0;0;0;False;0;False;0;0;-1;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;295;-1477.098,1169.576;Inherit;False;Global;WindSpeed;Wind Speed;6;0;Create;True;0;0;0;False;0;False;1;0;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;320;-1474.614,1279.88;Inherit;False;Global;WindTurbulence;Wind Turbulence;7;0;Create;True;0;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;308;-735.2812,1252.978;Inherit;False;Global;WindIntensity;Wind Intensity;5;0;Create;True;0;0;0;False;0;False;0.2;0;0;1;0;1;FLOAT;0
...
WireConnection;386;6;383;0
WireConnection;386;8;384;0
ASEEND*/
//CHKSM=11651164837A78FEE801B88B842D04A83012C946