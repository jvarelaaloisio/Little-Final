...
public class CharacterMovement2D : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
        ...
        // set sprinting
	    if ((UnityEngine.Input.GetKeyDown(sprintJoystick) || UnityEngine.Input.GetKeyDown(sprintKeyboard)) && input != Vector2.zero) isSprinting = true;
	    if ((UnityEngine.Input.GetKeyUp(sprintJoystick) || UnityEngine.Input.GetKeyUp(sprintKeyboard)) || input == Vector2.zero) isSprinting = false;
        anim.SetBool("isSprinting", isSprinting);

        // Jump
	    if ((UnityEngine.Input.GetKeyDown(jumpJoystick) || UnityEngine.Input.GetKeyDown(jumpKeyboard)) && isGrounded())
	    {
